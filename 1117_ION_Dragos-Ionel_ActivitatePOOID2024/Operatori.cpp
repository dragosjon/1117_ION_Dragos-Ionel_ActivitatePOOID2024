...
public:
	...
	//Să se supraîncarce operator de cast la int care determină numărul de persoane urcate deja în autobuz. 
	operator int() const {
		return nrPersoaneImbarcate;
	}

	//Să se supraîncarce operator> care va compara două autobuze după capacitatea acestora ( OPERATOR DE COMPARATIE )

	bool operator>(const Autobuz& a) {
		return this->capacitate > a.capacitate;
	}

	//sa se supraincarce  operatorul () functie care determina numarul de persoane care mai pot fi imbarcate pana la atingerea capacitatii maxime a autobuzului
...
int main() {
	...
	cout << "Numar locuri libere: " << autobuz3.getNumarLocuriLibere() << "; " << endl;
	...
	//testare Supraincarcarea operatorului ()
	cout << "Numarul de persoane care mai pot fi imbarcate in autobuz este: " << autobuz2() << endl;
